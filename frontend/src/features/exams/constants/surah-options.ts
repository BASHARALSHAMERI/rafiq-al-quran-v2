...
export const getSurahAyahCount = (surahId?: number | null): number => {
  if (!surahId || surahId < 1 || surahId > SURAH_AYAH_COUNTS.length) {
    return 0;
  }

  return SURAH_AYAH_COUNTS[surahId - 1];
};

export const SURAH_OPTIONS: SurahOption[] = SURAH_NAMES.map((name, index) => ({
  id: index + 1,
  name
}));

export const getSurahLabel = (surahId?: number | null): string => {
  if (!surahId || surahId < 1 || surahId > SURAH_OPTIONS.length) {
    return "غير محدد";
  }

  const surah = SURAH_OPTIONS[surahId - 1];
